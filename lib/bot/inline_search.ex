defmodule LoPrice.Bot.InlineSearch do
  ...
  defmacro __using__(_) do
    quote do
      def handle({:inline_query, %{query: query, offset: offset}}, context),
        do: LoPrice.Bot.InlineSearch.handle_inline_query(query, offset, context)
    end
  end
  ...
  def handle_inline_query(query, offset, context) do
    ...
    ExGram.Dsl.answer_inline_query(context, suggestions,
      is_personal: true,
      next_offset: "#{page + 1}"
    )
  end

  defp sber_suggestion_to_inline(%{
         "price" => price,
         "product" => %{
           "permalink" => permalink,
           "name" => product_name,
       ...
       do: %InlineQueryResultPhoto{
         ...
         thumb_url: mini_url,
         caption:
           "#{product_name} — <b>#{Product.format_price(price)}</b>\nhttps://sbermarket.ru/metro/#{
             permalink
           }",
         title: product_name,
  ...
  defp sber_product_to_inline(%{
         "price" => price,
         "name" => product_name,
         "sku" => sku,
         "images" => [%{"original_url" => original_url, "small_url" => mini_url} | _]
       }),
       do: %InlineQueryResultArticle{
         id: sku,
         type: "article",
         thumb_url: mini_url,
         thumb_width: 150,
           ...
           ExGram.Dsl.create_inline([
             [
               %{
                 ...
                 url:
                   "https://t.me/LocalLoPriceBot?start=#{
                     Base.encode64(sber_product_url_from_sku(sku))
                   }"
               }
             ]
           ])
