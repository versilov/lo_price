defmodule LoPrice.PriceChecker do
      ...
      |> Task.async_stream(
        fn %{
          ...
          if sber_product = SberMarket.find_lowest_price_in_stores(permalink, store_ids) do
            current_price =
              sber_product["offer"]["unit_price"] &&
                Product.to_kop(sber_product["offer"]["unit_price"])
            ...
            if sber_product["offer"]["active"] && current_price && current_price != last_price do
              ...
              if current_price < target_price && current_price < last_price &&
                   ((last_price - current_price) / last_price > 0.03 || last_price > target_price) do
  ...
  defp stream(user_id) when is_integer(user_id),
    do: Repo.stream(from(m in Monitor, where: m.user_id == ^user_id))

  defp maybe_send_notifications_groups(_, false), do: :ok

  defp maybe_send_notifications_groups({:ok, results}, true),
    do:
      results
